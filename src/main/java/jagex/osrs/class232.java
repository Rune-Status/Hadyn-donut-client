package jagex.osrs;

public class class232 extends Node {

    public static boolean field2665;
    public static class202 field2661;
    public static class202 field2663;
    public static class202 field2662;
    public static class249 field2757;
    public static class202 field2664;

    static {
        field2661 = new class202(200);
        field2662 = new class202(50);
        field2663 = new class202(20);
        field2664 = new class202(8);
        field2665 = false;
    }

    public int field2703;
    public String[] field2731;
    public int field2700;
    public int[] field2762;
    public boolean field2685;
    public int field2805;
    public int field2727;
    public int[] field2738;
    public int field2702;
    public int field2740;
    public int field2682;
    public int field2755;
    public boolean field2753;
    public int field2808;
    public int field2770;
    public int field2683;
    public int field2707;
    public int field2804;
    public boolean field2709;
    public int field2787;
    public int field2672;
    public int field2736;
    public boolean field2671;
    public int field2670;
    public int[] field2792;
    public int field2676;
    public int field2678;
    public int field2680;
    public int field2706;
    public int field2741;
    public int field2675;
    public int field2673;
    public int field2751;
    public int field2679;
    public int field2772;
    public boolean field2704;
    public int field2684;
    public int field2668;
    public int field2714;
    public int field2715;
    public class232[] field2791;
    public int field2723;
    public int[] field2786;
    public int[][] field2785;
    public int field2750;
    public int field2713;
    public int[] field2778;
    public int field2698;
    public int field2732;
    public int field2719;
    public int field2795;
    public Object[] field2782;
    public int field2788;
    public int field2711;
    public int field2689;
    public int field2690;
    public int field2691;
    public int field2686;
    public int field2693;
    public int field2749;
    public int field2694;
    public int field2799;
    public int field2695;
    public boolean field2696;
    public class321 field2744;
    public int field2699;
    public int field2768;
    public boolean field2701;
    public boolean field2807;
    public String field2728;
    public int field2780;
    public boolean field2705;
    public Object[] field2712;
    public int field2681;
    public Object[] field2763;
    public int field2776;
    public boolean field2784;
    public int field2730;
    public int field2718;
    public int field2717;
    public int field2720;
    public int field2734;
    public int field2721;
    public int field2722;
    public boolean field2716;
    public int field2708;
    public int[] field2726;
    public boolean field2688;
    public int[] field2737;
    public boolean field2725;
    public int field2793;
    public int field2797;
    public String field2729;
    public int field2794;
    public String[] field2739;
    public boolean field2724;
    public String field2697;
    public class232 field2748;
    public String field2752;
    public String field2677;
    public String field2790;
    public boolean field2798;
    public boolean field2669;
    public int field2800;
    public int field2801;
    public int field2802;
    public int field2735;
    public boolean field2746;
    public Object[] field2754;
    public Object[] field2774;
    public byte[][] field2710;
    public Object[] field2759;
    public Object[] field2761;
    public int[] field2806;
    public int[] field2765;
    public Object[] field2742;
    public int[] field2745;
    public Object[] field2764;
    public Object[] field2656;
    public Object[] field2773;
    public Object[] field2758;
    public Object[] field2756;
    public Object[] field2766;
    public byte[][] field2743;
    public Object[] field2660;
    public Object[] field2658;
    public int[] field2767;
    public Object[] field2803;
    public Object[] field2667;
    public Object[] field2760;
    public int[] field2769;
    public int[] field2771;
    public Object[] field2775;
    public Object[] field2666;
    public Object[] field2777;
    public Object[] field2747;
    public Object[] field2779;
    public Object[] field2789;
    public Object[] field2781;
    public Object[] field2783;
    public Object[] field2796;
    int field2687;
    int field2692;

    public class232() {
        this.field2685 = false;
        this.field2770 = -1;
        this.field2787 = -1;
        this.field2670 = 0;
        this.field2741 = 0;
        this.field2672 = 0;
        this.field2673 = 0;
        this.field2736 = 0;
        this.field2675 = 0;
        this.field2676 = 0;
        this.field2751 = 0;
        this.field2678 = 0;
        this.field2679 = 0;
        this.field2680 = 0;
        this.field2772 = 0;
        this.field2682 = 0;
        this.field2683 = 0;
        this.field2684 = 1;
        this.field2668 = 1;
        this.field2700 = -1;
        this.field2704 = false;
        this.field2711 = 0;
        this.field2689 = 0;
        this.field2690 = 0;
        this.field2691 = 0;
        this.field2686 = 0;
        this.field2693 = 0;
        this.field2694 = 0;
        this.field2695 = 0;
        this.field2696 = false;
        this.field2744 = class321.field3866;
        this.field2698 = 0;
        this.field2699 = 0;
        this.field2768 = 1;
        this.field2701 = false;
        this.field2702 = -1;
        this.field2703 = -1;
        this.field2780 = 0;
        this.field2705 = false;
        this.field2706 = 0;
        this.field2707 = 0;
        this.field2740 = 1;
        this.field2755 = -1;
        this.field2687 = 1;
        this.field2692 = -1;
        this.field2714 = -1;
        this.field2715 = -1;
        this.field2718 = 0;
        this.field2717 = 0;
        this.field2732 = 0;
        this.field2719 = 0;
        this.field2720 = 0;
        this.field2721 = 100;
        this.field2722 = 0;
        this.field2723 = 0;
        this.field2688 = false;
        this.field2725 = false;
        this.field2797 = 2;
        this.field2727 = -1;
        this.field2728 = "";
        this.field2729 = "";
        this.field2730 = 0;
        this.field2681 = 0;
        this.field2776 = 0;
        this.field2716 = false;
        this.field2734 = 0;
        this.field2708 = 0;
        this.field2713 = 0;
        this.field2724 = false;
        this.field2697 = "";
        this.field2748 = null;
        this.field2749 = 0;
        this.field2799 = 0;
        this.field2784 = false;
        this.field2752 = "";
        this.field2753 = false;
        this.field2788 = -1;
        this.field2677 = "";
        this.field2790 = "Ok";
        this.field2793 = -1;
        this.field2794 = 0;
        this.field2795 = 0;
        this.field2750 = 0;
        this.field2798 = false;
        this.field2669 = false;
        this.field2800 = -1;
        this.field2801 = 0;
        this.field2802 = 0;
        this.field2735 = 0;
        this.field2804 = -1;
        this.field2805 = -1;
        this.field2807 = false;
        this.field2746 = false;
    }

    public static String method4743(CharSequence var0, class330 var1, byte var2) {
        if (var0 == null) {
            return null;
        } else {
            int var3 = 0;

            int var4;
            for (var4 = var0.length();
                 var3 < var4 && class267.method5220(var0.charAt(var3), -2143990061); ++var3) {
                ;
            }

            while (var4 > var3 && class267.method5220(var0.charAt(var4 - 1), -2088972460)) {
                --var4;
            }

            int var5 = var4 - var3;
            if (var5 >= 1 && var5 <= class8.method97(var1, (byte) -39)) {
                StringBuilder var6 = new StringBuilder(var5);

                for (int var7 = var3; var7 < var4; ++var7) {
                    char var8 = var0.charAt(var7);
                    boolean var9;
                    if (Character.isISOControl(var8)) {
                        var9 = false;
                    } else if (class254.method5002(var8, (byte) 15)) {
                        var9 = true;
                    } else {
                        char[] var10 = class304.field3788;
                        int var11 = 0;

                        label73:
                        while (true) {
                            char var12;
                            if (var11 >= var10.length) {
                                var10 = class304.field3789;

                                for (var11 = 0; var11 < var10.length; ++var11) {
                                    var12 = var10[var11];
                                    if (var12 == var8) {
                                        var9 = true;
                                        break label73;
                                    }
                                }

                                var9 = false;
                                break;
                            }

                            var12 = var10[var11];
                            if (var8 == var12) {
                                var9 = true;
                                break;
                            }

                            ++var11;
                        }
                    }

                    if (var9) {
                        char var13 = class99.method2426(var8, (short) 11873);
                        if (var13 != 0) {
                            var6.append(var13);
                        }
                    }
                }

                if (var6.length() == 0) {
                    return null;
                } else {
                    return var6.toString();
                }
            } else {
                return null;
            }
        }
    }

    public class323 method4741(boolean var1, int var2) {
        field2665 = false;
        int var3;
        if (var1) {
            var3 = this.field2703;
        } else {
            var3 = this.field2702;
        }

        if (var3 == -1) {
            return null;
        } else {
            long var4 = ((long) this.field2707 << 40) + ((this.field2709 ? 1L : 0L) << 39) + (
                (this.field2671 ? 1L : 0L) << 38) + ((long) this.field2706 << 36) + (long) var3;
            class323 var6 = (class323) field2661.method4198(var4);
            if (var6 != null) {
                return var6;
            } else {
                var6 = class145.method3349(class289.field3709, var3, 0, (byte) -16);
                if (var6 == null) {
                    field2665 = true;
                    return null;
                } else {
                    if (this.field2671) {
                        var6.method6181();
                    }

                    if (this.field2709) {
                        var6.method6159();
                    }

                    if (this.field2706 > 0) {
                        var6.method6157(this.field2706);
                    }

                    if (this.field2706 >= 1) {
                        var6.method6160(1);
                    }

                    if (this.field2706 >= 2) {
                        var6.method6160(16777215);
                    }

                    if (this.field2707 != 0) {
                        var6.method6161(this.field2707);
                    }

                    field2661.method4204(var6, var4);
                    return var6;
                }
            }
        }
    }

    void method4701(Buffer var1, int var2) {
        var1.getUByte();
        this.field2685 = true;
        this.field2808 = var1.getUByte();
        this.field2741 = var1.readUint16();
        this.field2676 = var1.method3814((byte) -1);
        this.field2751 = var1.method3814((byte) -1);
        this.field2678 = var1.readUint16();
        if (this.field2808 == 9) {
            this.field2679 = var1.method3814((byte) -1);
        } else {
            this.field2679 = var1.readUint16();
        }

        this.field2736 = var1.method3931(2078136925);
        this.field2675 = var1.method3931(2141951688);
        this.field2672 = var1.method3931(2061363587);
        this.field2673 = var1.method3931(2070037344);
        this.field2700 = var1.readUint16();
        if (this.field2700 == 65535) {
            this.field2700 = -1;
        } else {
            this.field2700 += this.field2770 & -65536;
        }

        this.field2704 = var1.getUByte() == 1;
        if (this.field2808 == 0) {
            this.field2690 = var1.readUint16();
            this.field2691 = var1.readUint16();
            this.field2807 = var1.getUByte() == 1;
        }

        if (this.field2808 == 5) {
            this.field2702 = var1.getInt();
            this.field2780 = var1.readUint16();
            this.field2705 = var1.getUByte() == 1;
            this.field2698 = var1.getUByte();
            this.field2706 = var1.getUByte();
            this.field2707 = var1.getInt();
            this.field2671 = var1.getUByte() == 1;
            this.field2709 = var1.getUByte() == 1;
        }

        if (this.field2808 == 6) {
            this.field2740 = 1;
            this.field2755 = var1.readUint16();
            if (this.field2755 == 65535) {
                this.field2755 = -1;
            }

            this.field2718 = var1.method3814((byte) -1);
            this.field2717 = var1.method3814((byte) -1);
            this.field2732 = var1.readUint16();
            this.field2719 = var1.readUint16();
            this.field2720 = var1.readUint16();
            this.field2721 = var1.readUint16();
            this.field2714 = var1.readUint16();
            if (this.field2714 == 65535) {
                this.field2714 = -1;
            }

            this.field2688 = var1.getUByte() == 1;
            var1.readUint16();
            if (this.field2736 != 0) {
                this.field2722 = var1.readUint16();
            }

            if (this.field2675 != 0) {
                var1.readUint16();
            }
        }

        if (this.field2808 == 4) {
            this.field2727 = var1.readUint16();
            if (this.field2727 == 65535) {
                this.field2727 = -1;
            }

            this.field2728 = var1.method3778(-453480170);
            this.field2730 = var1.getUByte();
            this.field2681 = var1.getUByte();
            this.field2776 = var1.getUByte();
            this.field2716 = var1.getUByte() == 1;
            this.field2686 = var1.getInt();
        }

        if (this.field2808 == 3) {
            this.field2686 = var1.getInt();
            this.field2696 = var1.getUByte() == 1;
            this.field2698 = var1.getUByte();
        }

        if (this.field2808 == 9) {
            this.field2768 = var1.getUByte();
            this.field2686 = var1.getInt();
            this.field2701 = var1.getUByte() == 1;
        }

        this.field2713 = var1.method3773(357209724);
        this.field2697 = var1.method3778(-373382323);
        int var3 = var1.getUByte();
        if (var3 > 0) {
            this.field2731 = new String[var3];

            for (int var4 = 0; var4 < var3; ++var4) {
                this.field2731[var4] = var1.method3778(355010194);
            }
        }

        this.field2749 = var1.getUByte();
        this.field2799 = var1.getUByte();
        this.field2784 = var1.getUByte() == 1;
        this.field2752 = var1.method3778(-1208278349);
        this.field2754 = this.method4704(var1, 1475274853);
        this.field2759 = this.method4704(var1, 1632508415);
        this.field2761 = this.method4704(var1, -169303029);
        this.field2742 = this.method4704(var1, 799429101);
        this.field2764 = this.method4704(var1, 180640817);
        this.field2766 = this.method4704(var1, -1386543630);
        this.field2660 = this.method4704(var1, -352207187);
        this.field2667 = this.method4704(var1, 1369625207);
        this.field2658 = this.method4704(var1, 1373704144);
        this.field2773 = this.method4704(var1, -1854738646);
        this.field2760 = this.method4704(var1, 1036055867);
        this.field2656 = this.method4704(var1, 971210319);
        this.field2803 = this.method4704(var1, -329507416);
        this.field2756 = this.method4704(var1, 2080866575);
        this.field2758 = this.method4704(var1, 65508800);
        this.field2712 = this.method4704(var1, 128827589);
        this.field2763 = this.method4704(var1, -749757975);
        this.field2774 = this.method4704(var1, -1299907242);
        this.field2767 = this.method4703(var1, 1860958215);
        this.field2769 = this.method4703(var1, 1620755164);
        this.field2771 = this.method4703(var1, 1717316912);
    }

    void method4702(Buffer var1, int var2) {
        this.field2685 = false;
        this.field2808 = var1.getUByte();
        this.field2670 = var1.getUByte();
        this.field2741 = var1.readUint16();
        this.field2676 = var1.method3814((byte) -1);
        this.field2751 = var1.method3814((byte) -1);
        this.field2678 = var1.readUint16();
        this.field2679 = var1.readUint16();
        this.field2698 = var1.getUByte();
        this.field2700 = var1.readUint16();
        if (this.field2700 == 65535) {
            this.field2700 = -1;
        } else {
            this.field2700 += this.field2770 & -65536;
        }

        this.field2788 = var1.readUint16();
        if (this.field2788 == 65535) {
            this.field2788 = -1;
        }

        int var3 = var1.getUByte();
        int var4;
        if (var3 > 0) {
            this.field2786 = new int[var3];
            this.field2778 = new int[var3];

            for (var4 = 0; var4 < var3; ++var4) {
                this.field2786[var4] = var1.getUByte();
                this.field2778[var4] = var1.readUint16();
            }
        }

        var4 = var1.getUByte();
        int var5;
        int var6;
        int var7;
        if (var4 > 0) {
            this.field2785 = new int[var4][];

            for (var5 = 0; var5 < var4; ++var5) {
                var6 = var1.readUint16();
                this.field2785[var5] = new int[var6];

                for (var7 = 0; var7 < var6; ++var7) {
                    this.field2785[var5][var7] = var1.readUint16();
                    if (this.field2785[var5][var7] == 65535) {
                        this.field2785[var5][var7] = -1;
                    }
                }
            }
        }

        if (this.field2808 == 0) {
            this.field2691 = var1.readUint16();
            this.field2704 = var1.getUByte() == 1;
        }

        if (this.field2808 == 1) {
            var1.readUint16();
            var1.getUByte();
        }

        if (this.field2808 == 2) {
            this.field2762 = new int[this.field2678 * this.field2679];
            this.field2792 = new int[this.field2679 * this.field2678];
            var5 = var1.getUByte();
            if (var5 == 1) {
                this.field2713 |= 268435456;
            }

            var6 = var1.getUByte();
            if (var6 == 1) {
                this.field2713 |= 1073741824;
            }

            var7 = var1.getUByte();
            if (var7 == 1) {
                this.field2713 |= Integer.MIN_VALUE;
            }

            int var8 = var1.getUByte();
            if (var8 == 1) {
                this.field2713 |= 536870912;
            }

            this.field2734 = var1.getUByte();
            this.field2708 = var1.getUByte();
            this.field2726 = new int[20];
            this.field2737 = new int[20];
            this.field2738 = new int[20];

            int var9;
            for (var9 = 0; var9 < 20; ++var9) {
                int var10 = var1.getUByte();
                if (var10 == 1) {
                    this.field2726[var9] = var1.method3814((byte) -1);
                    this.field2737[var9] = var1.method3814((byte) -1);
                    this.field2738[var9] = var1.getInt();
                } else {
                    this.field2738[var9] = -1;
                }
            }

            this.field2739 = new String[5];

            for (var9 = 0; var9 < 5; ++var9) {
                String var11 = var1.method3778(446384906);
                if (var11.length() > 0) {
                    this.field2739[var9] = var11;
                    this.field2713 |= 1 << var9 + 23;
                }
            }
        }

        if (this.field2808 == 3) {
            this.field2696 = var1.getUByte() == 1;
        }

        if (this.field2808 == 4 || this.field2808 == 1) {
            this.field2681 = var1.getUByte();
            this.field2776 = var1.getUByte();
            this.field2730 = var1.getUByte();
            this.field2727 = var1.readUint16();
            if (this.field2727 == 65535) {
                this.field2727 = -1;
            }

            this.field2716 = var1.getUByte() == 1;
        }

        if (this.field2808 == 4) {
            this.field2728 = var1.method3778(-634920079);
            this.field2729 = var1.method3778(81517625);
        }

        if (this.field2808 == 1 || this.field2808 == 3 || this.field2808 == 4) {
            this.field2686 = var1.getInt();
        }

        if (this.field2808 == 3 || this.field2808 == 4) {
            this.field2693 = var1.getInt();
            this.field2694 = var1.getInt();
            this.field2695 = var1.getInt();
        }

        if (this.field2808 == 5) {
            this.field2702 = var1.getInt();
            this.field2703 = var1.getInt();
        }

        if (this.field2808 == 6) {
            this.field2740 = 1;
            this.field2755 = var1.readUint16();
            if (this.field2755 == 65535) {
                this.field2755 = -1;
            }

            this.field2687 = 1;
            this.field2692 = var1.readUint16();
            if (this.field2692 == 65535) {
                this.field2692 = -1;
            }

            this.field2714 = var1.readUint16();
            if (this.field2714 == 65535) {
                this.field2714 = -1;
            }

            this.field2715 = var1.readUint16();
            if (this.field2715 == 65535) {
                this.field2715 = -1;
            }

            this.field2721 = var1.readUint16();
            this.field2732 = var1.readUint16();
            this.field2719 = var1.readUint16();
        }

        if (this.field2808 == 7) {
            this.field2762 = new int[this.field2678 * this.field2679];
            this.field2792 = new int[this.field2678 * this.field2679];
            this.field2681 = var1.getUByte();
            this.field2727 = var1.readUint16();
            if (this.field2727 == 65535) {
                this.field2727 = -1;
            }

            this.field2716 = var1.getUByte() == 1;
            this.field2686 = var1.getInt();
            this.field2734 = var1.method3814((byte) -1);
            this.field2708 = var1.method3814((byte) -1);
            var5 = var1.getUByte();
            if (var5 == 1) {
                this.field2713 |= 1073741824;
            }

            this.field2739 = new String[5];

            for (var6 = 0; var6 < 5; ++var6) {
                String var12 = var1.method3778(415222434);
                if (var12.length() > 0) {
                    this.field2739[var6] = var12;
                    this.field2713 |= 1 << var6 + 23;
                }
            }
        }

        if (this.field2808 == 8) {
            this.field2728 = var1.method3778(197389761);
        }

        if (this.field2670 == 2 || this.field2808 == 2) {
            this.field2752 = var1.method3778(291041470);
            this.field2677 = var1.method3778(108833319);
            var5 = var1.readUint16() & 63;
            this.field2713 |= var5 << 11;
        }

        if (this.field2670 == 1 || this.field2670 == 4 || this.field2670 == 5
            || this.field2670 == 6) {
            this.field2790 = var1.method3778(-218237655);
            if (this.field2790.length() == 0) {
                if (this.field2670 == 1) {
                    this.field2790 = "Ok";
                }

                if (this.field2670 == 4) {
                    this.field2790 = "Select";
                }

                if (this.field2670 == 5) {
                    this.field2790 = "Select";
                }

                if (this.field2670 == 6) {
                    this.field2790 = "Continue";
                }
            }
        }

        if (this.field2670 == 1 || this.field2670 == 4 || this.field2670 == 5) {
            this.field2713 |= 4194304;
        }

        if (this.field2670 == 6) {
            this.field2713 |= 1;
        }

    }

    public class226 method4745(boolean var1, int var2) {
        if (this.field2703 == -1) {
            var1 = false;
        }

        int var3 = var1 ? this.field2703 : this.field2702;
        if (var3 == -1) {
            return null;
        } else {
            long var4 = ((long) this.field2707 << 40) + ((this.field2709 ? 1L : 0L) << 39) + (
                (this.field2671 ? 1L : 0L) << 38) + (long) var3 + ((long) this.field2706 << 36);
            class226 var6 = (class226) field2664.method4198(var4);
            if (var6 != null) {
                return var6;
            } else {
                class323 var7 = this.method4741(var1, -827486195);
                if (var7 == null) {
                    return null;
                } else {
                    class323 var8 = var7.method6154();
                    int[] var9 = new int[var8.field3887];
                    int[] var10 = new int[var8.field3887];

                    for (int var11 = 0; var11 < var8.field3887; ++var11) {
                        int var12 = 0;
                        int var13 = var8.field3879;

                        int var14;
                        for (var14 = 0; var14 < var8.field3879; ++var14) {
                            if (var8.field3889[var14 + var11 * var8.field3879] == 0) {
                                var12 = var14;
                                break;
                            }
                        }

                        for (var14 = var8.field3879 - 1; var14 >= var12; --var14) {
                            if (var8.field3889[var14 + var11 * var8.field3879] == 0) {
                                var13 = var14 + 1;
                                break;
                            }
                        }

                        var9[var11] = var12;
                        var10[var11] = var13 - var12;
                    }

                    var6 = new class226(var8.field3879, var8.field3887, var10, var9, var3);
                    field2664.method4204(var6, var4);
                    return var6;
                }
            }
        }
    }

    public class300 method4708(int var1) {
        field2665 = false;
        if (this.field2727 == -1) {
            return null;
        } else {
            class300 var2 = (class300) field2663.method4198((long) this.field2727);
            if (var2 != null) {
                return var2;
            } else {
                class249 var4 = class289.field3709;
                class249 var5 = class28.field282;
                int var6 = this.field2727;
                class300 var3;
                if (!class17.method212(var4, var6, 0, (byte) 1)) {
                    var3 = null;
                } else {
                    byte[] var8 = var5.method4831(var6, 0, 1789634852);
                    class300 var7;
                    if (var8 == null) {
                        var7 = null;
                    } else {
                        class300 var9 = new class300(var8, class324.field3890, class324.field3892,
                                                     class324.field3895, class258.field3334,
                                                     class324.field3894, class324.field3897);
                        class324.field3890 = null;
                        class324.field3892 = null;
                        class324.field3895 = null;
                        class258.field3334 = null;
                        class324.field3894 = null;
                        class324.field3897 = null;
                        var7 = var9;
                    }

                    var3 = var7;
                }

                if (var3 != null) {
                    field2663.method4204(var3, (long) this.field2727);
                } else {
                    field2665 = true;
                }

                return var3;
            }
        }
    }

    public class121 method4710(class275 var1, int var2, boolean var3, class229 var4, byte var5) {
        field2665 = false;
        int var6;
        int var7;
        if (var3) {
            var6 = this.field2687;
            var7 = this.field2692;
        } else {
            var6 = this.field2740;
            var7 = this.field2755;
        }

        if (var6 == 0) {
            return null;
        } else if (var6 == 1 && var7 == -1) {
            return null;
        } else {
            class121 var8 = (class121) field2662.method4198((long) (var7 + (var6 << 16)));
            if (var8 == null) {
                class115 var9;
                if (var6 == 1) {
                    var9 = class115.method2843(class132.field1858, var7, 0);
                    if (var9 == null) {
                        field2665 = true;
                        return null;
                    }

                    var8 = var9.method2772(64, 768, -50, -10, -50);
                }

                if (var6 == 2) {
                    var9 = class248.method4823(var7, -1265755379).method5430(-26261165);
                    if (var9 == null) {
                        field2665 = true;
                        return null;
                    }

                    var8 = var9.method2772(64, 768, -50, -10, -50);
                }

                if (var6 == 3) {
                    if (var4 == null) {
                        return null;
                    }

                    var9 = var4.method4671(520490175);
                    if (var9 == null) {
                        field2665 = true;
                        return null;
                    }

                    var8 = var9.method2772(64, 768, -50, -10, -50);
                }

                if (var6 == 4) {
                    class271 var10 = class84.method2125(var7, -205103429);
                    var9 = var10.method5381(10, (short) -19539);
                    if (var9 == null) {
                        field2665 = true;
                        return null;
                    }

                    var8 = var9
                        .method2772(var10.field3548 + 64, var10.field3531 * 5 + 768, -50, -10, -50);
                }

                field2662.method4204(var8, (long) (var7 + (var6 << 16)));
            }

            if (var1 != null) {
                var8 = var1.method5453(var8, var2, -2066035701);
            }

            return var8;
        }
    }

    public class323 method4709(int var1, int var2) {
        field2665 = false;
        if (var1 >= 0 && var1 < this.field2738.length) {
            int var3 = this.field2738[var1];
            if (var3 == -1) {
                return null;
            } else {
                class323 var4 = (class323) field2661.method4198((long) var3);
                if (var4 != null) {
                    return var4;
                } else {
                    var4 = class145.method3349(class289.field3709, var3, 0, (byte) -37);
                    if (var4 != null) {
                        field2661.method4204(var4, (long) var3);
                    } else {
                        field2665 = true;
                    }

                    return var4;
                }
            }
        } else {
            return null;
        }
    }

    Object[] method4704(Buffer var1, int var2) {
        int var3 = var1.getUByte();
        if (var3 == 0) {
            return null;
        } else {
            Object[] var4 = new Object[var3];

            for (int var5 = 0; var5 < var3; ++var5) {
                int var6 = var1.getUByte();
                if (var6 == 0) {
                    var4[var5] = new Integer(var1.getInt());
                } else if (var6 == 1) {
                    var4[var5] = var1.method3778(-122311369);
                }
            }

            this.field2753 = true;
            return var4;
        }
    }

    public void method4711(int var1, String var2, byte var3) {
        if (this.field2731 == null || this.field2731.length <= var1) {
            String[] var4 = new String[var1 + 1];
            if (this.field2731 != null) {
                for (int var5 = 0; var5 < this.field2731.length; ++var5) {
                    var4[var5] = this.field2731[var5];
                }
            }

            this.field2731 = var4;
        }

        this.field2731[var1] = var2;
    }

    public void method4706(int var1, int var2, byte var3) {
        int var4 = this.field2762[var2];
        this.field2762[var2] = this.field2762[var1];
        this.field2762[var1] = var4;
        var4 = this.field2792[var2];
        this.field2792[var2] = this.field2792[var1];
        this.field2792[var1] = var4;
    }

    int[] method4703(Buffer var1, int var2) {
        int var3 = var1.getUByte();
        if (var3 == 0) {
            return null;
        } else {
            int[] var4 = new int[var3];

            for (int var5 = 0; var5 < var3; ++var5) {
                var4[var5] = var1.getInt();
            }

            return var4;
        }
    }
}
